...
function Cart() {
  ...
  const handleQuantityChange = async (cartId, newQuantity) => {
    try {
      const response = await api.put("/api/cart/editCart", { cartId, quantity: newQuantity });
  ...
  const handleRemoveItem = async (cartId) => {
    try {
      const response = await api.delete("/api/cart/deleteCartFromMain", { data: { cartId } });
      setMessage(response.data.message);
  ...
  <span className="truncate w-32 block" title={item.productName}>{item.productName}</span>
        ...
        <div className="mt-6 w-60 p-4 border rounded-lg">
          ...
          <button
            className="mt-4 bg-red-500 text-white px-6 py-2 rounded-lg cursor-pointer"
            onClick={() => {
              const items = cartItems.map(i => ({ name: i.productName, price: Number(i.price), quantity: Number(i.quantity) }))
