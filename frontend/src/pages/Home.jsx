...
const Home = ({ searchQuery }) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
  const response = await api.get("/api/sellerProductUpload/getAllProducts");
  setProducts(response.data);
  // set dynamic max price for filters based on fetched products
  const maxP = response.data.reduce((m, p) => Math.max(m, Number(p.productOfferPrice || 0)), 0) || 2000;
  setMaxPrice(maxP);
  setPriceRange(maxP);
      } catch (err) {
        console.error('Fetch products error:', err);
        const serverMsg = err.response?.data?.msg || err.response?.data?.error || err.message;
        setError("Failed to fetch products: " + (serverMsg || 'Unknown error'));
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      if (token) {
        try {
          const response = await api.get("/api/userLogin/me");
        ...
        {filteredProducts.length === 0 ? (
          <div className="col-span-full text-center py-8">
            ...
            <button
              ...
              onClick={() => { setLoading(true); setError(null); (async ()=>{ try{ const r=await api.get('/api/sellerProductUpload/getAllProducts'); setProducts(r.data); }catch(e){ console.error(e); setError(e.response?.data?.error||e.message) } finally{ setLoading(false) } })() }}
            >Retry</button>
          </div>
        ) : (
