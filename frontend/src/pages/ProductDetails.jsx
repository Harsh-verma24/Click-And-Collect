...
const ProductDetails = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const response = await api.get(`/api/sellerProductUpload/getProduct/${id}`);
    ...
    const checkIfLiked = async () => {
      if (!token) return;
      try {
        const response = await api.get("/api/like/getLike");
    ...
    const checkIfInCart = async () => {
      if (!token) return;
      try {
        const response = await api.post("/api/cart/checkCart", { productId: id });
  ...
  const handleLikeToggle = async () => {
    ...
    try {
      if (isLiked) {
        await api.delete("/api/like/removeLike", { data: { productId: id } });

        setIsLiked(false);
        setMessage("Removed from wishlist.");
      } else {
        await api.post("/api/like/addLike", { productId: id });
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      const response = await api.post("/api/cart/addCart", cartData, {
  ...
  const handleRemoveFromCart = async () => {
    ...
    try {
      const response = await api.delete("/api/cart/deleteCart", { data: { cartId: id } });
