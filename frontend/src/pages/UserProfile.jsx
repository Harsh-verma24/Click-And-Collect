...
const UserProfile = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await api.get('/api/userProfile/getUserData');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let response;
      if (userData) {
  response = await api.put('/api/userProfile/editUserData', formData, config);
        setUserData(response.data.updateData);
      } else {
  response = await api.post('/api/userProfile/newUserData', formData, config);
